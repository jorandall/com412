...
# Depth-first search method for tree
  def dfs(self, node, search):
    if (node.value == search):
      return node.value

    if (node.value > search) and (node.left is not None):
        return self.dfs(node.left, search)
    elif (node.value < search) and (node.right is not None):
      return self.dfs(node.right, search)
